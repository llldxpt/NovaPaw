...
def _ensure_python_dir_group_acl() -> None:
    """Grants read/execute access to NovapawUsers on the Python install dir.
...
def _find_reusable_sandbox(
    ...
    sb_dir = _sandboxes_dir(_novapaw_state_dir)
...
def _create_new_sandbox(
    ...
    meta_path = _save_sandbox_metadata(
        _sandboxes_dir(_novapaw_state_dir),
        sandbox_id,
        meta,
    )
...
def _remove_acls_from_metadata(
    ...
    for entry in acl_entries:
        ...
        if sid_type == "cap":
            ...
            # Traverse ACEs for NovapawUsers group are persistent
