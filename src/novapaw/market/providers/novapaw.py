...
class NovaPawProvider:
    ...
    async def search(
        ...
        try:
            ...
            raise RuntimeError(
                f"NovaPaw search returned HTTP {e.response.status_code}",
            ) from e
        if not isinstance(body, dict) or not body.get("success", True):
            ...
            raise RuntimeError(f"NovaPaw search failed: {message}")
