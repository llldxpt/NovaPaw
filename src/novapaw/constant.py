...
# 1. NOVAPAW_WORKING_DIR env var is set → use it
# 2. ~/.novapaw exists (legacy installation) → use it as-is
# 3. Default → ~/.novapaw
_explicit_working_dir = EnvVarLoader.get_str("NOVAPAW_WORKING_DIR")
...
NOVAPAW_DESKTOP_PORT = EnvVarLoader.get_str("NOVAPAW_DESKTOP_PORT")
...
try:
    TOOL_GUARD_APPROVAL_HEARTBEAT_INTERVAL = max(
        float(
            EnvVarLoader.get_str(
                "NOVAPAW_TOOL_GUARD_APPROVAL_HEARTBEAT_INTERVAL",
                "15",
            ),
        ),
        5.0,
...
try:
    CAPABILITY_CACHE_TTL_SECONDS = max(
        float(
            EnvVarLoader.get_str(
                "NOVAPAW_CAPABILITY_CACHE_TTL_SECONDS",
                "1800",
            ),
        ),
        0.0,
