...
State file layout follows the snarktank/ralph convention:
    ...
    {project_dir}/.novapaw/missions/{loop_id}/
...
def ensure_mission_git_exclude(
    ...
    try:
        ...
        logger.warning(
            "Unable to exclude .novapaw Mission state in %s",
            project_dir,
            exc_info=True,
        )
        return False
...
def create_loop_dir(project_dir: Path) -> Path:
    ...
    loop_dir = project_dir / ".novapaw" / "missions" / loop_id
...
def get_active_loop_dir(
    ...
    """
    base = project_dir / ".novapaw" / "missions"
    if not base.exists():
        return None
