...
from .policy import is_novapaw_state_path
...
logger = logging.getLogger("novapaw.checkpoints")
...
class RestoreService:
    ...
    def _normalize_selected_file(cls, rel: str, *, conv_rel: str) -> str:
        ...
        if not cls._is_file_restore_candidate(normalized, conv_rel=conv_rel):
            raise CheckpointError(
                f"`--files` cannot restore NovaPaw state path `{rel}`.",
            )
        return normalized
