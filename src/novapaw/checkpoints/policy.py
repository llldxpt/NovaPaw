...
GIT_REQUIRED_MESSAGE = (
    "NovaPaw checkpoints require Git, but the git executable was not found on "
    "PATH. Install Git from https://git-scm.com/downloads, ensure `git` is "
    "available in your terminal, and then restart NovaPaw."
)
...
# Other NovaPaw-owned paths are runtime implementation details. Keep this
...
NOVAPAW_RUNTIME_STATE_FILES = frozenset(
    {
        ".bootstrap_completed",
        ".reme_store_v1",
        ".skill.json.lock",
...
NOVAPAW_STATE_SUFFIXES = (
    ".json.tmp",
    ".lock",
    ".weixin-migrate.bak",
)

# Public aggregate constants answer "is this NovaPaw-owned state?". They
# deliberately include checkpoint-owned session and memory paths.
NOVAPAW_STATE_FILES = frozenset(
    {*CHECKPOINT_STATE_FILES, *NOVAPAW_RUNTIME_STATE_FILES},
)
NOVAPAW_STATE_DIRS = (
    *CHECKPOINT_STATE_DIRS,
    *NOVAPAW_RUNTIME_STATE_DIRS,
...
# categories. These are not NovaPaw state, but checkpointing them would be
...
EXCLUDE_PATTERNS = (
    ".git/",
    *DEVELOPMENT_ARTIFACT_PATTERNS,
    *(f"/{name}" for name in sorted(NOVAPAW_RUNTIME_STATE_FILES)),
    *(f"/{name}" for name in NOVAPAW_RUNTIME_STATE_DIRS),
    *(f"/*{suffix}" for suffix in NOVAPAW_STATE_SUFFIXES),
)
...
def is_novapaw_state_path(rel: str) -> bool:
    """Return whether *rel* is NovaPaw runtime state, not user content."""
    normalized = (rel or "").replace("\\", "/").lstrip("/")
    if not normalized:
        return False
    if "/" not in normalized and normalized in NOVAPAW_STATE_FILES:
        return True
    if "/" not in normalized and normalized.endswith(NOVAPAW_STATE_SUFFIXES):
        return True
    return any(normalized.startswith(prefix) for prefix in NOVAPAW_STATE_DIRS)
...
def session_file_path(
    ...
    *,
    ...
    """Return the NovaPaw 2.0 JSON session path for one session."""
    if not session_id:
...
def session_key(*, channel: str, user_id: str, session_id: str) -> str:
    """Return a bounded, collision-resistant key for a NovaPaw session."""
...
def latest_user_query(
    ...
    *,
    ...
    """Return the latest persisted user text for one NovaPaw 2.0 session."""
