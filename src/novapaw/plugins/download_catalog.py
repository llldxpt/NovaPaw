...
def _get_version_constraint(
    ...
    """Extract a structured ``novapaw_version`` from a CDN index entry.

    Reuses the same normalization logic as
    ``scripts/pack/generate_plugin_metadata.get_version``:
      1. If ``novapaw_version`` dict exists, use directly.
      2. Otherwise synthesize from ``min_version`` / ``max_version``.

    Returns ``None`` when no version constraint is declared.
    """
    novapaw_version = entry.get("novapaw_version")
    if isinstance(novapaw_version, dict):
        return {
            k: _normalize_ver(str(v))
            for k, v in novapaw_version.items()
            if k in ("min", "max")
        }
...
def _is_entry_compatible(entry: dict[str, Any]) -> bool:
    """Return True when a CDN index entry supports the running NovaPaw.

    Checks the structured ``novapaw_version`` field first, then falls
    ...
    manifest_data: dict[str, Any] = {
        "id": plugin_id,
        "version": version,
        "novapaw_version": version_constraint,
    }
