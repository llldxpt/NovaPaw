# -*- coding: utf-8 -*-
"""Extensible hooks for `novapaw doctor`
(entry points + programmatic registration).

Plugins can expose a setuptools entry point in group ``novapaw.doctor``::

    [project.entry-points."novapaw.doctor"]
    my_pkg = "my_pkg.doctor:doctor_notes"

The group ``novapaw.doctor`` is still discovered when present (legacy plugins).
...
def _load_entry_point_functions() -> list[tuple[str, DoctorNotesFn]]:
    ...
    seen: set[str] = set()
    for group in ("novapaw.doctor", "novapaw.doctor"):
