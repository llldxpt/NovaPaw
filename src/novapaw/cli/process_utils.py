...
def _matches_novapaw_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a NovaPaw CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m novapaw {subcommand}",
            f" novapaw {subcommand}",
            f"__main__.py {subcommand}",
            f'novapaw.exe" {subcommand}',
            f"novapaw.exe {subcommand}",
        )
    )


def _is_novapaw_service_command(command: str) -> bool:
    """Return whether the command line looks like a local NovaPaw app."""
    return _matches_novapaw_cli_command(command, "app")


def _is_novapaw_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a NovaPaw CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "novapaw" or _matches_novapaw_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local NovaPaw service."""
    if not host:
        return []
