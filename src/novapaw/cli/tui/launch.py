# -*- coding: utf-8 -*-
"""Launch the NovaPaw TUI.

``novapaw``                    open an interactive chat with the active agent
``novapaw tui``                same, with explicit options
``novapaw tui --agent NAME``   chat with a specific agent
``novapaw tui --resume ID``    resume a previous session and continue it

The TUI spawns ``novapaw acp`` using the *current* interpreter
(``python -m novapaw acp --local-diagnostics``), so it always drives the same
install/venv it ships in -- no reliance on ``novapaw`` being on ``PATH``.

Textual and the transport are imported lazily so ``novapaw --help`` and other
...
def _build_transport(
    *,
    ...
    ``[sys.executable, "-m", "novapaw", "acp", "--local-diagnostics"]`` --
    ...
    *not* appended here: ``AcpTransport`` appends ``--agent <id>`` itself when
    ...
    description = (
        f"novapaw acp ({sys.executable} -m novapaw acp --local-diagnostics)"
    )
    if project_dir:
...
def tui_cmd(
    ...
    """Open the NovaPaw terminal chat UI."""
