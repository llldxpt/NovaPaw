...
* ``AgentMessageChunk`` / ``AgentThoughtChunk`` carry a single content block
  whose ``.text`` is already a *delta* (the NovaPaw ACP server emits deltas).
...
def _is_local_file_uri(uri: str) -> bool:
    ...
    ``resource_link`` could drive a one-click browser open. NovaPaw only emits
...
# ``_meta`` key NovaPaw sets on an ``agent_message_chunk`` to mark it as an
# error; mirrors the ACP server's ``ACP_ERROR_META_KEY``.
_ERROR_META_KEY = "novapaw.error"
...
def _tool_links(content: Any) -> tuple[FileLink, ...]:
    ...
    that carry a ``uri``), e.g. the link NovaPaw emits for
    ``send_file_to_user``.
    """
    if not content:
        return ()
...
def normalize_update(update: Any) -> list[TuiEvent]:
    ...
    if kind == "agent_message_chunk":
        meta = getattr(update, "field_meta", None)
        # NovaPaw reports per-call token usage as an (otherwise empty)
        ...
        if not text:
            return []
        # NovaPaw tags failed turns via ``_meta`` so we can render them as
