# -*- coding: utf-8 -*-
"""Conservative filesystem repairs for ``novapaw doctor fix``.
...
``src/novapaw/console/`` (needs network for npm).

``validate-all-jobs-json`` reuses
:func:`~novapaw.cli.doctor_checks.check_cron_jobs_files` (read-only); exits
...
from ..utils.console_static import find_novapaw_source_repo_root
    ...
    *,
        ...
        for fid in fix_ids:
            if fid in RISKY_FIX_IDS and not yes and not dry_run:
                raise ValueError(
                    ...
                    "Use `novapaw doctor fix --dry-run --only ...` to preview "
                    "the plan without -y.",
                )
    ...
    if (
        ...
        raise ValueError(
            ...
            "ensure-working-dir in --only or run `novapaw doctor fix` without "
    ...
    if "rebuild-console-npm" in fix_ids:
        ...
        if repo is None:
            skip_msgs.append(
                "rebuild-console-npm: only in a NovaPaw source checkout "
            ...
            def _rebuild_console(
                ...
                if (
                    ...
                    bkp_root = r / ".novapaw-doctor-fix-backups"
                    sid = _utc_session_id()
                    bkp = bkp_root / sid
