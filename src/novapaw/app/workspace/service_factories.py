...
async def create_driver_service(
    ...
    from ..mail.driver_config import (
        is_managed_novapawmail_card,
        sync_novapawmail_driver_card,
    )

    # Upgrade legacy novapawmail cards before DriverManager can launch them.
    ...
    existing_mail_card = (
        ws.workspace_dir / "drivers" / "mcp" / "novapawmail.yaml"
    )
    ...
    if should_sync_mail_card and not await asyncio.to_thread(
        ...
        ws.workspace_dir,
        mail,
        getattr(ws._config, "backend", "novapaw"),
    ):
        logger.warning(
            "novapawmail DriverCard could not be synchronized for agent %s; "
