# -*- coding: utf-8 -*-
"""Secret-free novapawmail DriverCard generation."""
...
import yaml
from novapawmail_mcp.providers import ENTERPRISE_PROVIDERS
...
_MAIL_MCP_MODULE_ARGS = ["-m", "novapawmail_mcp"]
_INTERNAL_MAIL_MCP_ARGS = ["--internal-mail-mcp"]


def is_managed_novapawmail_card(path: Path) -> bool:
    """Return whether *path* is a NovaPaw-generated mail DriverCard."""
    if not path.is_file():
        return False
    try:
    ...
    return bool(
        ...
        and endpoint.get("args") == ["-m", "novapawmail_mcp"],
    )


def resolve_novapawmail_endpoint() -> tuple[str, list[str]]:
    """Resolve the command and arguments for the novapawmail MCP server."""
    override = os.environ.get("NOVAPAWMAIL_PYTHON", "").strip()
    if override:
        return override, list(_MAIL_MCP_MODULE_ARGS)
    if getattr(sys, "frozen", False):
        executable = Path(sys.executable)
        cli_executable = executable.with_name(f"novapaw{executable.suffix}")
        return str(cli_executable), list(_INTERNAL_MAIL_MCP_ARGS)
    try:
        if importlib.util.find_spec("novapawmail_mcp") is not None:
...
def resolve_novapawmail_command() -> str:
    """Resolve the executable used to launch the novapawmail MCP server."""
    return resolve_novapawmail_endpoint()[0]


def _load_managed_novapawmail_card(path: Path) -> DriverCard | None:
    """Load an existing managed card whose user policy must be retained."""
    if not is_managed_novapawmail_card(path):
        return None
    try:
        return load_card(path)
    except DriverCardError as exc:
        logger.warning(
            "Failed to preserve invalid novapawmail DriverCard %s: %s",
...
def build_novapawmail_env(
    mail: AgentMailConfig | None,
    workspace_dir: Path | None = None,
) -> dict[str, Any]:
    """Build secret-free DriverCard env bindings for novapawmail.
    ...
    env: dict[str, Any] = {
        "NOVAPAWMAIL_EMAIL": f"{credential.name}@{credential.domain}",
        "NOVAPAWMAIL_AUTH_CODE": {
            "source": "credential",
            "credential": "mail",
            "field": "auth_code",
        },
    ...
    if profile is not None:
        env["NOVAPAWMAIL_IMAP_HOST"] = profile.imap_host
        env["NOVAPAWMAIL_IMAP_PORT"] = str(profile.imap_port)
        env["NOVAPAWMAIL_SMTP_HOST"] = profile.smtp_host
        env["NOVAPAWMAIL_SMTP_PORT"] = str(profile.smtp_port)
    if workspace_dir is not None:
        env["NOVAPAWMAIL_STATE_DIR"] = str(workspace_dir / "mail_state")
        env["NOVAPAWMAIL_WORKSPACE_DIR"] = str(workspace_dir)
    return env


def generate_novapawmail_driver_card(
    workspace_dir: Path,
    mail: AgentMailConfig | None = None,
) -> bool:
    """Persist mail secrets and publish a secret-free novapawmail card."""
    try:
        card_path = workspace_dir / "drivers" / "mcp" / "novapawmail.yaml"
        existing = _load_managed_novapawmail_card(card_path)
        (workspace_dir / "mail_state").mkdir(parents=True, exist_ok=True)
        save_agent_mail_credentials(workspace_dir, mail)
        env = build_novapawmail_env(mail, workspace_dir)
        command, args = resolve_novapawmail_endpoint()
        ...
        logger.warning(
            "Failed to generate novapawmail driver card for %s: %s",
...
def sync_novapawmail_driver_card(
    workspace_dir: Path,
    mail: AgentMailConfig | None,
    backend: str,
    *,
    ...
    embedded ``NOVAPAWMAIL_AUTH_CODE`` as a plaintext scalar.
    """
    del force_rewrite  # Compatibility with existing callers.
    driver_card = workspace_dir / "drivers" / "mcp" / "novapawmail.yaml"
    if backend != "novapaw" or mail is None:
        driver_card.unlink(missing_ok=True)
        save_agent_mail_credentials(workspace_dir, None)
        return True
    if not generate_novapawmail_driver_card(workspace_dir, mail):
