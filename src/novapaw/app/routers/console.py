...
from novapaw.utils.timeout import resolve_stream_task_timeout
...
def _resolve_effective_stream_task_timeout(
    ...
    Thin wrapper over :func:`novapaw.utils.timeout.resolve_stream_task_timeout`
...
async def _finalize_background_fork(
    ...
    *,
    ...
    ``completed``.
    """
    from novapaw.agents.fork_project import finalize_fork_worktree_or_fail
...
async def _mark_background_fork_failed(
    ...
    *,
    ...
    if not project_dir or not branch:
        return
    try:
        from novapaw.agents.fork_project import mark_fork_failed
