...
by the previous launch (verified to actually be a NovaPaw backend, to guard
...
def _looks_like_backend(proc: psutil.Process) -> bool:
    """Best-effort check that *proc* is a NovaPaw desktop backend.
    ...
    try:
        ...
        name = ""
    if "novapaw-backend" in name:
        return True
    try:
        exe = (proc.exe() or "").lower()
    except (psutil.Error, OSError):
        exe = ""
    if "novapaw-backend" in exe:
        return True
    try:
...
def _terminate_previous_backend(pid_file: Path) -> None:
    ...
    if not _looks_like_backend(proc):
        logger.info(
            "Recorded backend pid %s is not a NovaPaw backend "
            "(likely PID reuse); leaving it alone",
            pid,
        )
        return

    logger.info("Terminating orphaned NovaPaw backend pid %s", pid)
    try:
        proc.terminate()
        try:
