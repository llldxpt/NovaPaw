...
def _abort_unhandled_multiprocessing_child(args: Sequence[str]) -> None:
    ...
    if sys.stderr is not None:
        print(
            "novapaw-backend multiprocessing runtime hook did not handle "
            "the child process",
            file=sys.stderr,
        )
    raise SystemExit(2)
...
def _install_desktop_runtime() -> None:
    ...
    from novapaw.browser.runtime.managed_playwright import (
...
def _run_backend_server(log_level: str) -> None:
    ...
    from novapaw.browser.control_link.chrome.protocol import (
    ...
    from novapaw.app._app import app as fastapi_app
...
def main() -> None:
    ...
    from novapaw.utils.platform import warn_unelevated_sandbox
