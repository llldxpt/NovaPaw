...
NovaPaw's supported server topology uses one application worker and one
...
def get_path_lock(path: Path | str) -> asyncio.Lock:
    ...
    NovaPaw currently runs one application worker, so all supported writers
...
def get_sync_path_lock(path: Path | str) -> threading.RLock:
    ...
    Like :func:`get_path_lock`, this is sufficient for NovaPaw's current
