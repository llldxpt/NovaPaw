...
def _detect_install_method() -> str:
    """Detect how NovaPaw was installed based on environment signals."""
    from ..constant import EnvVarLoader

    if EnvVarLoader.get_bool("NOVAPAW_RUNNING_IN_CONTAINER"):
        return "docker"
    if EnvVarLoader.get_bool("NOVAPAW_DESKTOP_APP"):
        return "desktop"
    return "pip"
...
def get_system_info() -> dict[str, Any]:
    ...
    - novapaw_version: NovaPaw version string
    - install_method: How NovaPaw was installed (docker/desktop/pip)
    - os: Operating system (Windows/Darwin/Linux)
    - os_version: OS version string
    - python_version: Python version running novapaw (major.minor)
    - architecture: CPU architecture (x86_64/arm64/etc)
    - has_gpu: GPU availability detection
    """
    from ..__version__ import __version__ as novapaw_ver

    info = {
        "install_id": str(uuid.uuid4()),
        "novapaw_version": _safe_get(lambda: novapaw_ver, "unknown"),
...
def _get_current_version() -> str:
    """Get the current NovaPaw version string."""
    try:
        from ..__version__ import __version__ as novapaw_ver
...
def has_telemetry_been_collected(working_dir: Path) -> bool:
    ...
    Re-triggers collection when NovaPaw is upgraded (or downgraded) to a
    version that hasn't been collected before.

    Args:
        working_dir: Path to NovaPaw working directory
    ...
    try:
        ...
        # v1.1 compat: single novapaw_version field
        return marker_data.get("novapaw_version", "") == current
    except Exception:
        return False
        ...
        if marker_file.exists():
            try:
                ...
                if not collected_versions:
                    old_ver = old_data.get("novapaw_version", "")
                    if old_ver:
