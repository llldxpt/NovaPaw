...
_TOKEN_ENV = "NOVAPAW_RUNTIME_INTERNAL_TOKEN"
_TOKEN_HEADER = "X-NovaPaw-Runtime-Token"


def _runtime_url() -> httpx.URL | None:
    """Read the endpoint supplied by Hub, never a user API override."""
    runtime_id = EnvVarLoader.get_str("NOVAPAW_RUNTIME_ID")
    if not runtime_id or not EnvVarLoader.get_str(_TOKEN_ENV):
        return None
    value = EnvVarLoader.get_str("NOVAPAW_RUNTIME_API_URL")
    if not value:
        return None
    try:
        url = httpx.URL(value)
