...
_TRANSFORM_EXECUTOR = ThreadPoolExecutor(
    max_workers=2,
    thread_name_prefix="novapaw-visual-compact",
)
...
def _model_key(current_model: Any) -> str | None:
    """Return NovaPaw's exact per-model capability-cache key."""
    try:
...
def _agent_will_strip_media(
    ...
    """Whether NovaPaw will discard media for this exact model call."""
    formatter = getattr(agent, "formatter", None)
    if bool(getattr(formatter, "_novapaw_force_strip_media", False)):
        return True
