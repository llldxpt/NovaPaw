# -*- coding: utf-8 -*-
"""NovaPaw system/tool planning and atomic visual replacement."""
...
from .tool_schemas import plan_novapaw_tool_documentation
...
_ENV_TAIL_HEADER = (
    "[NovaPaw environment context supplied by the host; "
    "this text was not written by the user.]"
)
_ENV_TAIL_FOOTER = "[End NovaPaw environment context.]"
_NOVAPAW_ENV_BLOCK = re.compile(
    r"^====================\n"
...
def _extract_novapaw_env_context(text: str) -> tuple[str, str]:
    """Remove NovaPaw's complete env block from one system prompt."""
    matches = [
        match
        for match in _NOVAPAW_ENV_BLOCK.finditer(text)
        if all(marker in match.group(0) for marker in _NOVAPAW_ENV_MARKERS)
    ]
    if len(matches) != 1:
        return text, ""
    match = matches[0]
...
def _plan_system_partition(
    ...
    *,
    ...
    for index, message in enumerate(messages):
        ...
        if not env_tail:
            static_text, env_context = _extract_novapaw_env_context(text)
            if env_context:
...
def compress_static_context(  # pylint: disable=R0912,R0915
    ...
    *,
    ...
    new_tools = tools
    if tools:
        new_tools, tool_documentation = plan_novapaw_tool_documentation(tools)
    ...
    render_payload = (
        ...
        "===================\n"
        "NovaPaw rendered this session's configuration and tool parameter "
