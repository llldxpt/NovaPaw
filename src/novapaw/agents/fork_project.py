...
WORKTREE_REL = Path(".novapaw") / "worktrees"
REGISTRY_REL = Path(".novapaw") / "fork_registry.json"
# Written in the *agent workspace* so OMP gates can find the coding-project
# registry when ``project_dir != workspace_dir``.
INTEGRATION_PROJECT_REL = Path(".novapaw") / "fork_integration_project"
ACTIVE_SCOPE_REL = Path(".novapaw") / "fork_active_scope"
...
def project_dir_from_worktree(worktree_path: Path) -> Path:
    """``<project>/.novapaw/worktrees/<id>`` → ``<project>``."""
...
def _fork_finalize_lock_path(project_dir: Path, branch: str) -> Path:
    ...
    return project_dir / ".novapaw" / "fork_locks" / f"{digest}.lock"
...
def _ensure_git_identity(wt: Path) -> None:
    ...
    Disabled when ``NOVAPAW_FORK_AUTO_GIT_IDENTITY=0``. Prefers existing
    repo/global git config and does not overwrite a configured identity.
    """
    if os.environ.get("NOVAPAW_FORK_AUTO_GIT_IDENTITY", "1") == "0":
        return
    for key, value in (
        ("user.email", "novapaw-fork@localhost"),
        ("user.name", "NovaPaw Fork"),
    ):
