...
NOVAPAW_MODEL_INFO_ENV = "NOVAPAW_MODEL_INFO_JSON"
...
def _optional_int(
    ...
    if type(value) is not int or value < minimum:
        raise ValueError(
            f"{NOVAPAW_MODEL_INFO_ENV}.{name} must be an integer "
...
def _load_runtime_model_info(source: Mapping[str, str]) -> dict[str, Any]:
    ...
    raw = str(source.get(NOVAPAW_MODEL_INFO_ENV, "")).strip()
    if not raw:
        return {}
    try:
        ...
        raise ValueError(
            f"{NOVAPAW_MODEL_INFO_ENV} must be a JSON object",
        ) from exc
    if not isinstance(model_info, dict):
        raise ValueError(f"{NOVAPAW_MODEL_INFO_ENV} must be a JSON object")
    return model_info
