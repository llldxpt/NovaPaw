...
novapaw envelope schema and the ACP transport stack.  Eagerly importing
them here would slow down every consumer of ``novapaw.agents.acp`` —
including ``delegate_external_agent`` which only needs
``tool_adapter`` — so we expose ``NovaPawACPAgent`` /
``run_novapaw_agent`` / ``ACPService`` / ``*_acp_service`` via
...
_LAZY_EXPORTS = {
    "NovaPawACPAgent": ".server",
    "run_novapaw_agent": ".server",
    "ACPService": ".service",
...
# fine — verified by ``from novapaw.agents.acp import ACPService`` working.
__all__ = [
    "ACPErrors",
    "ACPConfigurationError",
    "ACPProtocolError",
