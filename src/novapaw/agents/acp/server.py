...
_ACP_RUNTIME_MODEL_SLOT_KEY = "novapaw.runtime_model_slot"
...
class NovaPawACPAgent(Agent):
    ...
    async def _ensure_workspace(self) -> Any:
        ...
        async with self._workspace_lock:
            ...
            logger.info(
                "NovaPaw ACP Agent workspace started:"
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise RequestError.internal_error(
                {"details": "NovaPaw runtime failed"},
            ) from None
        finally:
    ...
    async def _sync_session_mcp(
        ...
        if driver_manager is None:
            raise RuntimeError(
                "NovaPaw workspace has no DriverManager for ACP MCP",
            )
