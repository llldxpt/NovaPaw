...
from novapaw.utils.tool_call_extra import (
...
class OpenAIChatModelCompat(OpenAIChatModel):
    ...
    * ``provider_id`` — canonical NovaPaw provider identity used to scope
    ...
    * ``extra_generate_kwargs`` — merged into every ``_call_api`` invocation
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        credential_provider_id = credential_id.removeprefix("novapaw-")
        if (
            provider_id
        ...
        self._novapaw_provider_id = provider_id or credential_provider_id

    @property
    def novapaw_provider_id(self) -> str:
        """Canonical provider ID used for provider-scoped metadata."""
        return self._novapaw_provider_id

    def bind_novapaw_provider_id(self, provider_id: str) -> None:
        """Bind the provider identity resolved by ``ProviderManager``.

        Credential-derived IDs are only a fallback for models constructed
        outside NovaPaw's factory.  Inside the factory, the resolved provider
        instance is authoritative for metadata, accounting, and retry keys.
        """
        if not provider_id:
            raise ValueError("provider_id must not be empty")
        self._novapaw_provider_id = provider_id
    ...
    async def _relay_stream_tool_call_extras(
        ...
        ``ToolCallBlock``, but NovaPaw's transient attribute intentionally is
            ...
            if sanitized_response.extra_contents:
                for block in parsed.content:
                    ...
                    if ec:
                        attach_transient_tool_call_extra(
                            block,
                            provider_id=self._novapaw_provider_id,
                            extra_content=ec,
                        )
