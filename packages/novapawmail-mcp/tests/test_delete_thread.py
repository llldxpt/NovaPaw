...
from novapawmail_mcp.config import load_config
from novapawmail_mcp.errors import MailError
from novapawmail_mcp.server import create_server
from novapawmail_mcp.thread_store import ThreadStore
...
def _config():
    return load_config(
        {
            "NOVAPAWMAIL_EMAIL": "tester@163.com",
            "NOVAPAWMAIL_AUTH_CODE": "secret",
        },
    )
...
def test_partial_delete_keeps_failed_message_indexed(tmp_path):
    ...
    with patch.dict(
        "os.environ",
        {"NOVAPAWMAIL_STATE_DIR": str(tmp_path)},
    ):
