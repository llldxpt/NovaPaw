...
def resolve_state_dir(
    ...
    Uses the NOVAPAWMAIL_STATE_DIR environment variable when set (novapaw
    injects it, pointing at the agent workspace mail_state/ directory);
    otherwise falls back to ``~/.novapawmail-mcp/state/<email>/``.
    """
    ...
    return Path.home() / ".novapawmail-mcp" / "state" / safe
...
class ThreadStore:
    ...
    def process_transaction(self) -> Iterator[None]:
        ...
        NovaPaw's build-before-swap driver reload briefly runs the old and new
