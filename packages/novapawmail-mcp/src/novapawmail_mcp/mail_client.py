...
#: Env vars injected by novapaw into the MCP stdio subprocess.
WORKSPACE_DIR_ENV = "NOVAPAWMAIL_WORKSPACE_DIR"
STATE_DIR_ENV = "NOVAPAWMAIL_STATE_DIR"
...
def resolve_attachments_base(
    ...
    The MCP stdio subprocess inherits its CWD from the novapaw backend
    (the source repo root), so relative save paths must NOT be resolved
    against the CWD.  Priority:

    1. NOVAPAWMAIL_WORKSPACE_DIR - agent workspace root (newer cards);
    2. parent of NOVAPAWMAIL_STATE_DIR - STATE_DIR points at
       <workspace>/mail_state, so its parent is the workspace root
       (works for existing driver cards without regeneration);
    3. ~/.novapawmail-mcp/state/<email>/attachments - default, matching
    ...
    return Path.home() / ".novapawmail-mcp" / "state" / safe / "attachments"
...
_SMTP_RULES: tuple[
    ...
    (
        ...
        AuthError,
        "authentication failed. Verify NOVAPAWMAIL_AUTH_CODE is the correct "
...
def _map_imap_login_error(exc: Exception) -> MailError:
    ...
    if any(
        ...
        return AuthError(
            f"IMAP authentication failed ({text}). Verify NOVAPAWMAIL_EMAIL "
            "and NOVAPAWMAIL_AUTH_CODE. Depending on your provider, "
