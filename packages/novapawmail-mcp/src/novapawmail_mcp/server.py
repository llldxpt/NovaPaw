...
def create_server(
    ...
    def _get_store() -> ThreadStore:
        ...
        The state directory comes from the NOVAPAWMAIL_STATE_DIR environment
        variable (injected by novapaw); when missing it falls back to
        ~/.novapawmail-mcp/state/<email>/. Directories are auto-created.
    ...
    async def set_credentials(
        ...
        def _sync() -> dict:
            env = {
                "NOVAPAWMAIL_EMAIL": email,
                "NOVAPAWMAIL_AUTH_CODE": auth_code,
            }
            if imap_host:
                env["NOVAPAWMAIL_IMAP_HOST"] = imap_host
            if smtp_host:
                env["NOVAPAWMAIL_SMTP_HOST"] = smtp_host
