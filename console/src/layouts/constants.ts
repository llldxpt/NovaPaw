...
cd .. && mkdir -p src/novapaw/console
cp -R console/dist/. src/novapaw/console/
pip install -e .
\`\`\`
...
docker run -p 127.0.0.1:8088:8088 -v novapaw-data:/app/working -v novapaw-secrets:/app/working.secret -v novapaw-backups:/app/working.backups agentscope/qwenpaw:latest
\`\`\`
