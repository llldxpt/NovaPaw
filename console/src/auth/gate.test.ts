...
describe("authentication gate", () => {
  ...
  it("accepts only a token verified by the backend", async () => {
    localStorage.setItem("novapaw_auth_token", "window-a-token");
    getStatus.mockResolvedValueOnce({ enabled: true, has_users: true });
    const fetchSpy = vi
  ...
  it("clears a rejected token and requires login", async () => {
    localStorage.setItem("novapaw_auth_token", "expired-token");
    getStatus.mockResolvedValueOnce({ enabled: true, has_users: true });
    ...
    await expect(resolveAuthGate()).resolves.toBe("auth-required");
    expect(localStorage.getItem("novapaw_auth_token")).toBeNull();
  });

  it("fails closed and preserves the token on a service error", async () => {
    localStorage.setItem("novapaw_auth_token", "retry-token");
    getStatus.mockResolvedValueOnce({ enabled: true, has_users: true });
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
    ...
    expect(localStorage.getItem("novapaw_auth_token")).toBe("retry-token");
  });

  it("preserves the token when verification cannot reach the backend", async () => {
    localStorage.setItem("novapaw_auth_token", "window-b-token");
    getStatus.mockResolvedValueOnce({ enabled: true, has_users: true });
    vi.spyOn(globalThis, "fetch").mockRejectedValueOnce(new Error("offline"));

    await expect(resolveAuthGate()).rejects.toThrow("offline");
    expect(localStorage.getItem("novapaw_auth_token")).toBe("window-b-token");
  });
});
