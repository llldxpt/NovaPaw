 ...
 * which delegate to the host's existing NovaPaw namespace and APIs.
 */
...
export async function chat(
  ...
  const agentId =
    options.agentId ?? window.NovaPaw.host?.getSelectedAgentId?.() ?? "default";
  const sessionId =
    options.sessionId === undefined
      ? window.NovaPaw.host?.getCurrentSessionId?.() ?? undefined
      : options.sessionId ?? undefined;
...
/** Stream a chat turn as decoded NovaPaw envelope events. */
...
export async function toast(
  ...
  // Use NovaPaw host notification if available (same-origin)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const novapaw = (window as any).NovaPaw as
    | Record<string, unknown>
    | undefined;
  if (novapaw?.host) {
    const host = novapaw.host as {
...
export function createHostNamespace(
  ...
  return {
    ...
    getSelectedAgentId: () =>
      window.NovaPaw.host?.getSelectedAgentId?.() ?? "default",
    getCurrentSessionId: () =>
      window.NovaPaw.host?.getCurrentSessionId?.() ?? null,
    async toast(message, kind = "info") {
      const host = window.NovaPaw.host as {
