 ...
 *   window.NovaPaw.registerRoutes("cloudpaw", [{ path: "/a2a", ... }])
 *
...
describe("legacy registerRoutes shim", () => {
  it("attaches window.NovaPaw.{menu, route, slot, audit}", () => {
    expect(typeof window.NovaPaw.menu?.add).toBe("function");
    expect(typeof window.NovaPaw.route?.add).toBe("function");
    expect(typeof window.NovaPaw.slot?.fill).toBe("function");
    expect(typeof window.NovaPaw.audit?.overrides).toBe("function");
  });

  it("translates CloudPaw-shape registerRoutes into menu+route", () => {
    window.NovaPaw.registerRoutes!("cloudpaw", [
      {
        path: "/a2a",
        component: Comp,
        label: "A2A",
  ...
  it("multiple legacy registers share the synthesized plugins-group", () => {
    window.NovaPaw.registerRoutes!("p1", [
      { path: "/p1", component: Comp, label: "P1", icon: "1" },
    ]);
    window.NovaPaw.registerRoutes!("p2", [
  ...
  it("new NovaPaw.menu.add coexists with legacy in plugins-group", () => {
    window.NovaPaw.registerRoutes!("cloudpaw", [
      { path: "/a2a", component: Comp, label: "A2A", icon: "🔗" },
    ]);
    window.NovaPaw.menu!.add("newPlugin", {
      id: "newPlugin.foo",
  ...
  it("legacy registerToolRender still writes to pluginSystem", async () => {
    ...
    const RenderFC = () => null;
    window.NovaPaw.registerToolRender!("p1", { my_tool: RenderFC });
