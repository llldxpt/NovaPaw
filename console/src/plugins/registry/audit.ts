/**
 * registry/audit.ts — single override log shared by all NovaPaw registries
 * (console-wide menu/route/slot + chat extensions).
 *
 * Ring buffer (default 500 entries). Plugin authors can read via
 * `window.NovaPaw.audit.overrides()` for debugging / change attribution.
 *
...
class AuditStore {
  ...
  record(rec: OverrideRecord): void {
    ...
    if (
      ...
      console.warn(
        `[NovaPaw audit] ${rec.kind} ${id} by ${rec.pluginId}` +
