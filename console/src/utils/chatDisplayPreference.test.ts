...
describe("chatDisplayPreference", () => {
  beforeEach(() => {
    localStorage.removeItem("novapaw_tool_calls_default_expanded");
    localStorage.removeItem("novapaw_tool_display_mode");
    localStorage.removeItem("novapaw_assistant_message_display_mode");
    localStorage.removeItem("novapaw_show_thinking");
    vi.restoreAllMocks();
  });
  ...
  it("persists hidden thinking and clears the preference when shown", () => {
    ...
    expect(localStorage.getItem("novapaw_show_thinking")).toBe("false");

    setShowThinkingPreference(true);
    expect(getShowThinkingPreference()).toBe(true);
    expect(localStorage.getItem("novapaw_show_thinking")).toBeNull();
  });
  ...
  it("persists and clears the tool display preference", () => {
    ...
    expect(getToolDisplayPreference()).toBe("current");
    expect(localStorage.getItem("novapaw_tool_display_mode")).toBeNull();
  });

  it("clears the superseded tool expansion preference", () => {
    localStorage.setItem("novapaw_tool_calls_default_expanded", "true");

    setToolDisplayPreference("raw-input-output");

    expect(
      localStorage.getItem("novapaw_tool_calls_default_expanded"),
    ).toBeNull();
  });
  ...
  it("clears storage when result collapse is restored", () => {
    ...
    expect(getAssistantMessageDisplayPreference()).toBe("result-collapsed");
    expect(
      localStorage.getItem("novapaw_assistant_message_display_mode"),
    ).toBeNull();
  });

  it("ignores an invalid stored assistant display mode", () => {
    localStorage.setItem("novapaw_assistant_message_display_mode", "invalid");
