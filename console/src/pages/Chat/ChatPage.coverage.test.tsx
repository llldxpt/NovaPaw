...
const {
  ...
  mockRequiresNovaPawModel: vi.fn(() => true),
...
vi.mock("@/utils/agentBackend", () => ({
  requiresNovaPawModel: mockRequiresNovaPawModel,
...
describe("ChatPage coverage", () => {
  beforeEach(() => {
    ...
    mockRequiresNovaPawModel.mockReset();
    mockRequiresNovaPawModel.mockReturnValue(true);
  ...
  it("leaves the query unchanged for a non-NovaPaw backend", async () => {
    mockRequiresNovaPawModel.mockReturnValue(false);
    mockBeginLoopModeSubmission.mockImplementation(
      (text: string) => `/goal ${text}`,
    );
    renderWithProviders(<ChatPage />, {
  ...
  it("issue 7559: persists the follow-up while backend cleanup is running and sends it once idle", async () => {
    ...
    const storageKey = `novapaw:message-queue:${chatId}`;
    let statusChecks = 0;
