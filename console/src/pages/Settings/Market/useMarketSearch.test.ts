...
const PROVIDERS_KEY = "novapaw-market-providers";

const emptyResponse = { by_provider: {}, results: [], errors: [] };

const novapaw = { key: "novapaw", name: "NovaPaw", available: true };
...
describe("useMarketSearch", () => {
  beforeEach(() => {
    vi.useFakeTimers();
    localStorage.clear();
    mocks.listMarketProviders.mockReset().mockResolvedValue([novapaw, github]);
  ...
  it("loads providers on mount and falls back to novapaw selection", async () => {
    const { result } = await mount();
    expect(result.current.providers).toEqual([novapaw, github]);
    expect([...result.current.selectedProviderKeys]).toEqual(["novapaw"]);
  });
  ...
  it("drops restored providers that are no longer available", async () => {
    ...
    const { result } = await mount();
    // stale-key not enabled → fallback to novapaw
    expect([...result.current.selectedProviderKeys]).toEqual(["novapaw"]);
  });

  it("falls back to the first enabled provider when novapaw is unavailable", async () => {
    mocks.listMarketProviders.mockResolvedValue([
      { key: "aliyun", name: "Aliyun", available: true },
      { key: "novapaw", name: "NovaPaw", available: false },
    ]);
    const { result } = await mount();
  ...
  it("ignores corrupted localStorage selection gracefully", async () => {
    ...
    const { result } = await mount();
    // Falls back to novapaw without throwing
    expect([...result.current.selectedProviderKeys]).toEqual(["novapaw"]);
  });
  ...
  it("replaces results on a new search and sums provider totals", async () => {
    mocks.listMarketProviders.mockResolvedValue([novapaw, github]);
    localStorage.setItem(PROVIDERS_KEY, JSON.stringify(["novapaw", "github"]));
    mocks.searchMarket.mockResolvedValue({
      by_provider: {
        novapaw: { has_more: true, total: 25 },
  ...
  it("appends results on loadMore and exhausts when has_more is false", async () => {
    mocks.searchMarket
      .mockResolvedValueOnce({
        by_provider: { novapaw: { has_more: true, total: 20 } },
        results: [{ id: "page1" }],
        errors: [],
      })
      .mockResolvedValueOnce({
        by_provider: { novapaw: { has_more: false, total: 20 } },
        results: [{ id: "page2" }],
        errors: [],
      });
    const { result } = await mount();
    ...
    expect(mocks.searchMarket).toHaveBeenLastCalledWith(
      expect.objectContaining({ provider_pages: { novapaw: 2 } }),
    );
  ...
  it("blocks auto-loading after a batch with provider errors", async () => {
    mocks.searchMarket.mockResolvedValue({
      by_provider: { novapaw: { has_more: true, total: 5 } },
      results: [{ id: "r1" }],
      errors: [{ provider: "novapaw", error: "timeout" }],
    });
    const { result } = await mount();
    await flush();
  ...
  it("autoLoadMore is a no-op once cursors are exhausted", async () => {
    ...
    vi.useRealTimers();
    mocks.searchMarket.mockResolvedValue({
      by_provider: { novapaw: { has_more: false, total: 0 } },
      results: [],
      errors: [],
    });
    const { result } = await mount();
  ...
  it("manual loadMore re-arms auto-loading after an error block", async () => {
    mocks.searchMarket
      .mockResolvedValueOnce({
        by_provider: { novapaw: { has_more: true, total: 9 } },
        results: [],
        errors: [{ provider: "novapaw", error: "x" }],
      })
      .mockResolvedValueOnce({
        by_provider: { novapaw: { has_more: false, total: 9 } },
  ...
  it("retry refetches providers when none were loaded", async () => {
    ...
    expect(result.current.providers).toEqual([novapaw]);
  });
  ...
  it("refresh resets cursors and refetches from page 1", async () => {
    ...
    await flush();
    expect(mocks.searchMarket).toHaveBeenCalledWith(
      expect.objectContaining({ provider_pages: { novapaw: 1 } }),
    );
  });
});
