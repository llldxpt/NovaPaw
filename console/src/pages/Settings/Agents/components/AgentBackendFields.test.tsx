...
const { mockCopyText, mockMessage, mockProvider } = vi.hoisted(() => ({
  ...
  mockProvider: {
    ...
    runtime_path: "/opt/novapaw/site-packages/codex_cli_bin/bin/codex",
    ...
    capabilities: {
      ...
      native_mcp_ui: false,
      novapaw_skills_projection: true,
      novapaw_mcp_projection: true,
...
function BackendForm() {
  ...
  return (
    <Form form={form} initialValues={{ backend: "novapaw" }}>
...
describe("AgentBackendFields", () => {
  ...
  it("shows a compact detected path and copies the full path", async () => {
    ...
    expect(
      screen.getByText("/opt/novapaw/site-packages/codex_cli_bin/bin/codex"),
    ...
    await waitFor(() =>
      expect(mockCopyText).toHaveBeenCalledWith(
        "/opt/novapaw/site-packages/codex_cli_bin/bin/codex",
      ),
    );
